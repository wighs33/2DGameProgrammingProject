...
# 9. 게임의 간단한 소개
- 디펜스 게임으로 유닛으로 몬스터를 처치해서 라운드를 깨나가는 게임이다.
...
# 11. main game state 에 등장하는 game object
- *몬스터 클래스*
...
- *유닛 클래스*
  ...
  마우스로 원하는 유닛을 선택하여 이동시킨다.
  
  키를 누르면 몬스터에게 공격을 한다.

# 12. 어려웠던/어려운 부분
- 다운받은 이미지의 배경이 투명이 아니여서 투명으로 만드는 것이 어려웠습니다.

- 애니메이션 프레임별 이미지에 들어 있는 유닛의 좌표를 일일이 확인하기 번거로웠습니다.

- 유닛을 마우스 클릭으로 선택해서 선택한 유닛만 이동시키는 것이 어려웠고 해결했습니다.

- 유닛의 idle상태와 run상태의 이미지가 애니메이션의 프레임값이 달라서 어렵고 유닛들을 피클로 구현해야 할 것 같은데 어떻게 구현할지 감이 안옵니다.
